...
if __name__ == '__main__':
    # process('D:/new_work/raw/tencentvideo_review_raw.csv', 'D:/new_work/raw/tencentvideo_review.txt')
    #dataframe = pd.read_csv('newdata/QQmusic.csv', header=None)  # path是csv文件保存路径
    #dataframe.to_csv('newdata/1.csv', encoding='utf-8')
    #print(dataframe)
    #exit()
    print(111)
    process('newdata/zhilian.csv', 'reviews/zhilian.txt')
    #review=pd.read_csv('reviews/airbnb.csv')
    #print(review)
    exit()

    app_categories = ['business', 'life', 'education', 'music', 'entertainment', 'navigation', 'news', 'social', 'tool',
                      'travel']
    for app_category in app_categories:
        result_file = 'data/selected_test_reviews/未处理/new/{}_selected_reviews_test_supp_preprocess.csv'.format(app_category)
        #测试文件
        test_review_after_preprocess(result_file)
        # continue

        review_file = 'data/selected_test_reviews/未处理/new/{}_selected_reviews_test_supp.csv'.format(app_category)
        ...
        for review in review_list:
            ...
            if "----------------------------------------------------------------------------------------- " in review[
                4]:
                review[4] = review[4][:review[4].index(
                    '----------------------------------------------------------------------------------------- ')]
            # process each text
            title = re.sub('[^0-9A-Za-z\u4e00-\u9fa5]', '', review[3])
            content = review[4].strip()
            content = content.replace('\\', '')
            content = content.replace(',', '，')
            content = content.replace('.', '。')
            content = content.replace('\n', '。').strip('（该条评论已经被删除）')
            text = title + '，' + content
            csv_writer.writerow([0, text])
        if lines != 220:
            print('error', app_category)
